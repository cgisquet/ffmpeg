                                                   ...
                                                   int dc_shift)
{
    ...
    len = get_vlc2(&row->gb, ctx->dc_vlc.table, DNXHD_DC_VLC_BITS, 1);
    if (len < 0) {
        ret = len;
        goto error;
    }
    ...
    while (index1 != eob_index) {
        ...
        sign = -get_bits1(&row->gb);

        if (flags & 1) {
            level += get_bits(&row->gb, index_bits) << 7;
        }

        if (flags & 2) {
            index2 = get_vlc2(&row->gb, ctx->run_vlc.table, DNXHD_VLC_BITS, 2);
