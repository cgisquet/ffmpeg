...
static int read_len_table(uint8_t *dst, BitstreamContext *bc, int n)
{
    int i, val, repeat;
    ...
    if (s->bitstream_bpp < 24 || s->version > 2) {
        ...
        for (p = 0; p < count; p++) {
            ...
            if (s->version > 2 || s->bitstream_bpp == 12) {
                ...
                for (i = 0; i < s->vlc_n; i++) {
                    if (s->len[p][i] < 3) total++;
                }

                s->fast[p] = total > 2 ? 1 : 0;

                if (s->fast[p]) {
                    lut = ff_huff_joint4same_gen(&s->vlc[8 + p], symbols,
                                                 s->vlc_n, VLC_BITS,
                                                 s->bits[p0], s->bits[p],
                                                 s->len[p0], s->len[p], NULL, NULL);
                    if (!lut)
                ...
                for (i = 0; i < 1<<VLC_BITS; i++) {
                    if (s->fast[p] && s->vlc[8+p].table[i][1] > 0) {
                        s->mem[p][i].len  = s->vlc[8+p].table[i][1];
                        s->mem[p][i].type = 2;
                        s->mem[p][i].code.for4 = lut[s->vlc[8+p].table[i][0]];
...
static int read_huffman_tables(HYuvContext *s, const uint8_t *src, int length)
{
    ...
    for (i = 0; i < count; i++) {
        if ((ret = read_len_table(s->len[i], &bc, s->vlc_n)) < 0)
            return ret;
...
#define READ_2PIX(dst0, dst1, plane1)                                   \
    GET_VLC_DUAL(dst0, dst1, &s->bc, s->vlc[4+plane1].table,            \
...
static void decode_422_bitstream(HYuvContext *s, int count)
{
    ...
    icount = bitstream_bits_left(&s->bc) / (32 * 4);
    if (count >= icount) {
        for (i = 0; i < icount; i++) {
        ...
        for (; i < count && bitstream_bits_left(&s->bc) > 0; i++) {
            READ_2PIX(s->temp[0][2 * i    ], s->temp[1][i], 1);
            if (bitstream_bits_left(&s->bc) <= 0) break;
            READ_2PIX(s->temp[0][2 * i + 1], s->temp[2][i], 2);
        }
        for (; i < count; i++)
...
#define READ_2PIX_PLANE8(dst, off, plane) \
    GET_VLC_ITER2(dst, off, &s->bc, s->mem[plane], s->vlc[plane].table, VLC_BITS, 3)
...
static void decode_gray_bitstream(HYuvContext *s, int count)
{
    ...
    if (count >= bitstream_bits_left(&s->bc) / 32) {
        ...
        for (i = 0; i < count-2 && bitstream_bits_left(&s->bc) > 0;) {
            READ_2PIX_PLANE8(dst, i, 0);
        }
        }
    } else if (s->fast[0]) {
        for (i = 0; i < count-4;) {
...
#define READ_2PIX_PLANE(dst0, dst1, plane, OP) \
    /* //UPDATE_CACHE(re, &s->bc); */ \
    GET_VLC_DUAL(dst0, dst1, &s->bc, s->vlc[4+plane].table, \
...
static void decode_plane_bitstream(HYuvContext *s, int width, int plane)
{
    ...
    if (s->bps <= 8) {
        ...
        int count = width/2;
        if (count >= bitstream_bits_left(&s->bc) / (32 * 2)) {
