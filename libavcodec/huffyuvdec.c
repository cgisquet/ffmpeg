...
#define GET_VLC_ITER(dst, off, name, gb, JTable, table, bits, max_depth)   \
    do {                                                                   \
...
#define READ_4PIX_PLANE(dst, off, plane) \
    GET_VLC_ITER(dst, off, re, &s->gb, s->mem[plane], s->vlc[plane].table, VLC_BITS, 3)

#define GET_VLC_ITER2(dst, off, name, gb, JTable, table, bits, max_depth)  \
    do {                                                            \
        unsigned int index = SHOW_UBITS(name, gb, bits);            \
        int          code, n = JTable[index].len;                   \
                                                                    \
...
#define READ_2PIX_PLANE8(dst, off, plane) \
    GET_VLC_ITER2(dst, off, re, &s->gb, s->mem[plane], s->vlc[plane].table, VLC_BITS, 3)
...
static void decode_gray_bitstream(HYuvContext *s, int count)
{
    ...
    if (count >= (BITS_LEFT(re, &s->gb)) / 32) {
        if (s->fast[0]) {
        for (i = 0; i < count-4 && BITS_LEFT(re, &s->gb) > 0;) {
            READ_4PIX_PLANE(dst, i, 0);
        }
        } else {
        for (i = 0; i < count-2 && BITS_LEFT(re, &s->gb) > 0;) {
