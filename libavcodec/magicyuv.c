...
typedef struct MagicYUVContext {
    ...
    int (*huff_build)(VLC *vlc, VLC_MULTI *table, uint8_t *len, int mask);
...
static int huff_build10(VLC *vlc, VLC_MULTI *table, uint8_t *len, int mask)
{
    HuffEntry he[1024];
    uint32_t codes[1024];
    uint8_t bits[1024];
    ...
    return ff_huff_multi_gen(table, vlc, mask, VLC_BITS, codes, bits, lut, 2);
}

static int huff_build12(VLC *vlc, VLC_MULTI *table, uint8_t *len, int mask)
{
    HuffEntry he[4096];
    uint32_t codes[4096];
    uint8_t bits[4096];
...
static int huff_build(VLC *vlc, VLC_MULTI *table, uint8_t *len, int mask)
{
    HuffEntry he[256];
    uint32_t codes[256];
    uint8_t bits[256];
        ...
        if (flags & 1) {
            ...
            for (k = 0; k < height; k++) {
                if (width >= get_bits_left(&gb) / 32) {
                    for (x = 0; x < width-VLC_MULTI_MAX_SYMBOLS+1 && get_bits_left(&gb) > 0;) {
                        READ_2PIX_PLANE(dst, x, i, MULTI16b);
                    }
                } else {
                    for (x = 0; x < width-VLC_MULTI_MAX_SYMBOLS+1;) {
                        READ_2PIX_PLANE(dst, x, i, MULTI16b);
                    }
                }
    ...
    for (i = 0; i < s->planes; i++) {
        ...
        if (flags & 1) {
            ...
            for (k = 0; k < height; k++) {
                if (width >= get_bits_left(&gb) / 32) {
                    for (x = 0; x < width-VLC_MULTI_MAX_SYMBOLS+1 && get_bits_left(&gb) > 0;) {
                        READ_2PIX_PLANE(dst, x, i, MULTI8b);
                    }
                } else {
                    for (x = 0; x < width-VLC_MULTI_MAX_SYMBOLS+1;) {
                        READ_2PIX_PLANE(dst, x, i, MULTI8b);
...
static int build_huffman(AVCodecContext *avctx, GetBitContext *gbit, int max)
{
    ...
    while (get_bits_left(gbit) >= 8) {
        ...
        if (j == max) {
            j = 0;
            if (s->huff_build(&s->vlc[i], s->multi+i, s->len[i], s->vlc_n)) {
