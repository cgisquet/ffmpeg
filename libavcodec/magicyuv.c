...
typedef struct MagicYUVContext {
    ...
    int (*huff_build)(VLC *vlc, VLC *jvlc, uint8_t *len, int mask);
...
static int huff_build10(VLC *vlc, VLC *jvlc, uint8_t *len, int mask)
{
    HuffEntry he[1024];
    uint32_t codes[1024];
    uint8_t bits[1024];
...
static int huff_build12(VLC *vlc, VLC *jvlc, uint8_t *len, int mask)
{
    HuffEntry he[4096];
    uint32_t codes[4096];
    uint8_t bits[4096];
...
static int huff_build(VLC *vlc, VLC *jvlc, uint8_t *len, int mask)
{
    HuffEntry he[256];
    uint32_t codes[256];
    uint8_t bits[256];
...
static int build_huffman(AVCodecContext *avctx, GetBitContext *gbit, int max)
{
    ...
    while (get_bits_left(gbit) >= 8) {
        ...
        if (j == max) {
            j = 0;
            if (s->huff_build(&s->vlc[i], &s->vlc[i+4], s->len[i], s->vlc_n)) {
